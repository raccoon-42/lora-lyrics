...
\begin{frame}{How the judge scores them: higher score, worse lyrics}
  ...
  \begin{tabular}{@{}lcc@{}}
    ...
    \midrule
    \metric{attribution}   & .989 $\rightarrow$ .674 & .689 $\rightarrow$ .901 \\
    \metric{types}/sample  & 1.6 $\rightarrow$ 5.7 (3.6$\times$) & 2.5 $\rightarrow$ 3.3 (1.3$\times$) \\
    \metric{occ.}/type     & 1.2 $\rightarrow$ 2.1 & 1.2 $\rightarrow$ \textbf{8.6} \\
    \bottomrule
  \end{tabular}
  \take{A degenerate ``hey'' loop \emph{beats} a coherent verse. Tool's gain is pure repetition (\metric{types} flat).}
\end{frame}
