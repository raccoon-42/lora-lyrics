...
\begin{frame}{Baseline 1 --- pretrained \texttt{gemma-4-E4B}: junk}
...
\begin{frame}{Baseline 2 --- instruct \texttt{gemma-4-E4B-it}: fluent, but one voice}
...
\begin{frame}{Fine-tune the pretrained base: one QLoRA adapter per artist}
  \begin{itemize}
    \item Base: \textbf{Gemma~4~E4B} ($\sim$4B), QLoRA 4-bit NF4
...
\begin{frame}{Three ablations we run}
  ...
  \end{itemize}
  \vfill
  {\centering\footnotesize\color{junk} Score = mean over 10 samples per config\\ Shared generation seed\par}
\end{frame}
...
% ---------- 15. Close + thesis ----------
\begin{frame}{Embedding space agrees with the classifier}
  \begin{columns}[c]
    \begin{column}{0.55\textwidth}
      \centering
